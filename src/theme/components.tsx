...
function renderCSS(props: { [x: string]: any; }, currentBreakpoint: string) {
  if (!props) return '';
...
export function Icon({ as, styleSheet, ...props }: Props) {
  const {
    iconVariant,
    ...restStyleSheet
  } = styleSheet;
  ...
  return (
    <Box
      as={FontAwesomeIcon}
      icon={iconSet[`fa${capitalize(iconVariant)}` as keyof typeof iconSet]}
...
export function Text({ as, styleSheet, ...props }: Props) {
  const {
    textVariant = {
      fontSize: 'inherit',
    },
...
export function Image({ as, ...props }: Props) {
  const tag = as || 'img';
  const {
    children,
...
export function Input({ as, styleSheet, ...props }: Props) {
  const tag = 'input';
...
export function Button({ as, styleSheet, ...props }: Props) {
  const {
  ...
  const finalStyleSheet = {
    ...
    outline: 0,
    width: theme.space["x1/1"],
    border: `${theme.space.xpx} solid ${theme.colors[buttonVariant as keyof typeof theme.colors][900]}`,
    borderRadius: theme.space.x2,
    paddingHorizontal: {
      xs: theme.space.x5,
      sm: theme.space.x10
    ...
    paddingVertical: theme.space.x3,
    transition: 'all 0.2s ease-in-out',
    backgroundColor: theme.colors[buttonVariant as keyof typeof theme.colors][600],
    hover: {
      backgroundColor: theme.colors[buttonVariant as keyof typeof theme.colors][500],
      boxShadow: `0 5px 10px -5px ${theme.colors.neutral[999]}73`,
    },
    focus: {
      backgroundColor: theme.colors[buttonVariant as keyof typeof theme.colors][700],
